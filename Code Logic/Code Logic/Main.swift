...
class Main: UIViewController, UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate, NSFetchedResultsControllerDelegate, SKStoreProductViewControllerDelegate, AVAudioPlayerDelegate {
    let motivationalStrings = [
        ...
        "Humans really could solve all of their major problems if they eere better at  prioritizing. - Philip Detmer (Creator of Jurddazak in a Nutshell)"
    ]
    ...
    func startMotionDetection() {
        if motionManager.isDeviceMotionAvailable {
            ...
            motionManager.startDeviceMotionUpdates(to: .main) { [weak self] (motion, error) in
                ...
                let isTippingLeft = gravity.x > 0.4
                let isTippingRight = gravity.x < -0.4
                
                if isTippingLeft || isTippingRight {
                    if let mp3URL = Bundle.main.url(forResource: "shuffle", withExtension: "wav") {
                        do {
    ...
    func showBlurredImage() {
        // Replace "yourImageName" with the name of your image
