...
class Main: UIViewController, UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate, NSFetchedResultsControllerDelegate, SKStoreProductViewControllerDelegate, AVAudioPlayerDelegate, AVSpeechSynthesizerDelegate {
    ...
    let SongNames = [ "LIFI PowerPod", "Night Snow Cleats", "VIBROV", "Neutrophil Alert and SWAT", "Supreme Commander", "Cards Hidden Tables Turned", "Space Race", "DROID DOGLET", "Paranormal Starship", "Astrally Slipped Out"]
...
//            guard let self = self else { return }
    ...
    func Current_Companion() {
        if (CurrentActions < 3650 && selectedEntity == "ASTRAL") || (selectedEntity == nil) {
        ...
        if CurrentActions > 3650 && CurrentActions < 7300 && selectedEntity == "ASTRAL" {
            Scribble.layer.opacity = 1.0
        ...
        if CurrentActions > 7300 && CurrentActions < 10950 && selectedEntity == "ASTRAL" {
            Scribble.layer.opacity = 1.0
        ...
        if CurrentActions > 10950 && CurrentActions < 14600 && selectedEntity == "ASTRAL" {
            Scribble.layer.opacity = 1.0
        ...
        if CurrentActions > 18250 && selectedEntity == "ASTRAL" {
            Scribble.layer.opacity = 1.0
