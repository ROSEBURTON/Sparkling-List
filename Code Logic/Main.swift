...
class Main: UIViewController, UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate, NSFetchedResultsControllerDelegate, SKStoreProductViewControllerDelegate, AVAudioPlayerDelegate, AVSpeechSynthesizerDelegate {
    let motivators = [
        ...
        "Tumors do not deserve resources earned at the expense of exploiting, they should be starved out of the host",
...
//            let alertController = UIAlertController(title: "Subscribe for extra features?", message: "For a $3.99 month subscription you'll unlock premium features and gain access to benefits such as Custom Gradient Colors for both Text and Task Backgrounds", preferredStyle: .actionSheet)
//            let monthlySubscriptionAction = UIAlertAction(title: "Monthly Subscription of $3.99", style: .default) { _ in
//            }
//            alertController.addAction(monthlySubscriptionAction)
...
//                    print("Thank you for subscribing to the shop. You now have access to customizable list colors, fonts, and deletion sounds.")
//                case .failed:
//                    SKPaymentQueue.default().finishTransaction(transaction)
//                    isUserSubscribed = false
//                    print("For customers only. Subscribe to access customizable list colors, fonts, and deletion sounds.")
//                case .deferred, .purchasing:
//                    SKPaymentQueue.default().finishTransaction(transaction)
//                    print("Subscription was deferred.")
//                @unknown default:
...
//        print("To reach the OPTIMAL level you need to reach \(years_actions_goal) actions or more.")
    ...
    @objc func handleTap(_ sender: UITapGestureRecognizer) {
        ...
        let compliments = ["🌏", "🇧🇷", "🇸🇰", "🇯🇵", "🇰🇷", "Great job!", "Keep up the good work!", "You're doing fantastic!", "New Year New Me", "You're making progress!", "🍾 Sparkling! 🍾", "Resolutions here I come!", "Bring productive in style", "??¿¿??", "!!¡¡!!", "Ɑ+˥˥ ⱭⱭ ", "EE¡¡ƎƎ¡¡EE",  "<0>___<0>", "O_O", ";-)", ">:)", ">:D","=^.^=", "(O.O)", "<|-_-|>", "<|^_^|>", "<(>_<)>", "ψ(｀∇´)ψ", "(☉_☉)", "(◣_◢)"]
    ...
    func FlipAndDeleteCell(at indexPath: IndexPath, in tableView: UITableView) {
        ...
        let Noptions: UIView.AnimationOptions = .transitionFlipFromLeft

        UIView.transition(with: tableView.cellForRow(at: indexPath) ?? UITableViewCell(), duration: 0.2, options: options, animations: {

        }) { _ in
            UIView.transition(with: tableView.cellForRow(at: indexPath) ?? UITableViewCell(), duration: 0.2, options: Noptions, animations: {
    ...
    func UpDownAndDeleteCell(at indexPath: IndexPath, in tableView: UITableView) {
        ...
        UIView.transition(with: tableView.cellForRow(at: indexPath) ?? UITableViewCell(), duration: 0.3, options: options, animations: {
        }) { _ in
            let cell = tableView.cellForRow(at: indexPath)
