...
class Main: UIViewController, UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate, NSFetchedResultsControllerDelegate, SKStoreProductViewControllerDelegate, AVAudioPlayerDelegate {
    let motivationalStrings = [
        ...
        "You should do things based on how it will affect the next seven generations, not just the first one",
    ...
    let songNames = ["Chromed & Neutralized", "Distraught Commander", "Southern Dark N' Oily",  "Blue Waters", "Space Race"]
...
//            Make it so that if isSilent is true CurrentSong: UILabel! flashes red and clear
            print("OFF")
        } else {
            Song_Radio?.volume = 0.7
            print("ON")
    ...
    let colors: [UIColor] = [.red, .blue, .green] // Define your transitioning colors
    var currentIndex = 0
    ...
    func showBlurredImage() {
        ...
        UIView.animate(withDuration: 7.0, delay: 7.0, options: [], animations: {
...
//    }
        ...
        if indexPath.row < Combined_missions.count {
            ...
            if let missionEntity = task as? MissionEntity {
                ...
                Timer.scheduledTimer(withTimeInterval: 0.05, repeats: true) { timer in
                    ...
                    let newColor = self.transitionColor(from: currentColor, to: nextColor, progress: 0.1) // Adjust progress value as needed
    ...
    @IBAction func addTaskButtonTapped(_ sender: Any) {
        ...
        alertController.addTextField { (textField) in
            textField.placeholder = "Expand past comfort zone"
            textField.font = font
            textField.textColor = self.randomPastelColor()
            textField.delegate = self
            textField.textAlignment = .center
        ...
        alertController.addTextField { (textField) in
            textField.placeholder = "Replace something disliked"
            textField.font = font
            textField.textColor = self.randomPastelColor()
            textField.delegate = self
            textField.textAlignment = .center
