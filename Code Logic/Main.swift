...
var years_actions_max = 18250
var day_actions_max: Int {
    let calendar = Calendar.current
    let currentDayOfYear = calendar.ordinality(of: .day, in: .year, for: Date()) ?? 1
    let remainingDays = 365 - currentDayOfYear
    return calculateDayActionsMax(newYearActions: UserDefaults.standard.integer(forKey: "New_Year_Actions"), remainingDays: remainingDays)
}

func calculateDayActionsMax(newYearActions: Int, remainingDays: Int) -> Int {
    let remainingActions = max(years_actions_max - newYearActions, 0)
...
class Main: UIViewController, UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate, NSFetchedResultsControllerDelegate, SKStoreProductViewControllerDelegate, AVAudioPlayerDelegate {
    let motivationalStrings = [
        ...
        "The only way to do great work is to love what you do, reward yourself. - Steve Jobs",
        ...
        "Vegan-Related: You put a baby in a crib with an apple and a rabbit. If it eats the rabbit and plays with the apple, I'll buy you a new car. - Harvey Diamond",
        ...
        "Vegan-Related: I hold that the more helpless a creature, the more entitled it is to protection by humans from the cruelty of humans. - Mahatma Gandhi"
        ]
    ...
    var audio_ability: AVAudioPlayer?
    ...
    @IBAction func Mixing_Dough(_ sender: UIButton) {
        ...
        do {
            audio_ability = try AVAudioPlayer(contentsOf: soundURL)
            audio_ability?.prepareToPlay()
            audio_ability?.volume = 0.7
            audio_ability?.play()
        } catch {
        }
    ...
    func applicationDidBecomeActive(_ application: UIApplication) {
        ...
        let currentDate = Date()
        let lastOpenDate = userDefaults.value(forKey: "lastOpenDate") as? Date
        if let lastOpenDate = lastOpenDate, Calendar.current.isDateInToday(lastOpenDate) {
        } else {
            var openCount = userDefaults.integer(forKey: "openCount")
            openCount += 1
            userDefaults.set(openCount, forKey: "openCount")
            userDefaults.set(currentDate, forKey: "lastOpenDate")
        }
    }
    ...
    @objc func updateTextColor() {
        Points_Label.font = UIFont(name: selectedFont ?? "Chalkduster", size: 15.0)
        New_Years_Label.font = UIFont(name: selectedFont ?? "Chalkduster", size: 15.0)
        ...
        if points >= day_actions_max {
            UIView.transition(with: Points_Label, duration: transitionDuration, options: .transitionCrossDissolve, animations: {
                self.Points_Label.textColor = self.New_Years_Satisfied[self.colorIndex]
                self.colorIndex = (self.colorIndex + 1) % self.New_Years_Satisfied.count
            }, completion: nil)
        } else {
            UIView.transition(with: Points_Label, duration: transitionDuration, options: .transitionCrossDissolve, animations: {
                if self.DAY_transitioningToRed {
                    self.Points_Label.textColor = .clear
                } else {
                    self.Points_Label.textColor = .red
                }
        ...
        if let newYearActions = UserDefaults.standard.value(forKey: "New_Year_Actions") as? Int {
            let percentage = Double(newYearActions) / Double(years_actions_max) * 100.0
            New_Years_Label.text = "\(newYearActions) / \(years_actions_max) actions this year \(String(format: "%.2f", percentage))% Finished"
            let currentDate = Date()
            let calendar = Calendar.current
            if let dayOfYear = calendar.ordinality(of: .day, in: .year, for: currentDate) {
                ...
                if currentDayTimes50 <= newYearActions {
                    UIView.transition(with: New_Years_Label, duration: transitionDuration, options: .transitionCrossDissolve, animations: {
                        print("\n\nGreat job for the year, you current day times 50 is \(currentDayTimes50) and you met your quota at \(newYearActions)")
                        self.New_Years_Label.textColor = self.New_Years_Satisfied[self.colorIndex]
                        self.colorIndex = (self.colorIndex + 1) % self.New_Years_Satisfied.count
                    }, completion: nil)
                }
                else {
    ...
    func Current_AI_Presented() {
        ...
        if selectedEntity == "Sketches" {
            Current_AI.layer.zPosition = -1
            Current_AI.transform = CGAffineTransform(scaleX: 1.3, y: 1.3)
            GifPlayer().playGif(named: "SketchesVALID", in: Current_AI)
            Scribble.layer.opacity = 0.0
        }
    }
    ...
    //MARK: Play the beat !!
    let songNames = ["Chromed & Neutralized", "Distraught Commander", "Play Dice", "Southern Dark N' Oily", "The Waitress serves on this night", "天宫空间站出界", ]
    ...
    func playNextSong() {
        ...
        // Pause playback in case of interruption
        pausePlayback()

        do {
            Song_Voicebox = try AVAudioPlayer(contentsOf: URL(fileURLWithPath: audioPath))
            Song_Voicebox?.numberOfLoops = 0
            Song_Voicebox?.prepareToPlay()
            Song_Voicebox?.volume = 0.7
            Song_Voicebox?.delegate = self

            // Resume playback after interruption
            resumePlayback()

            // Start or resume playing the song
            Song_Voicebox?.play()
            CurrentSong.font = UIFont(name: selectedFont ?? "Chalkduster", size: 20)
        } catch {
    ...
    func pausePlayback() {
        // Check if the audio player is playing before pausing
        if let player = Song_Voicebox, player.isPlaying {
            player.pause()
        }
    }

    func resumePlayback() {
        // Check if the audio player is paused before resuming
        if let player = Song_Voicebox, !player.isPlaying {
            player.play()
        }
    }
...
//    @objc func Repeat_Execution() {
//        DispatchQueue.global(qos: .background).async {
//
//                var latestMissionValue: String?
//                let fetchRequest: NSFetchRequest<RepeatEntity> = RepeatEntity.fetchRequest()
//                
//                do {
//                    let repeatEntities = try self.context.fetch(fetchRequest)
//                    if let latestRepeatEntity = repeatEntities.last {
//                        latestMissionValue = latestRepeatEntity.repeat_1
//                    }
//
//                    // INSERTING REPEAT 1
//                    if let latestMissionValue = latestMissionValue {
//                        self.context.performAndWait {
//                            guard let entityDescription = NSEntityDescription.entity(forEntityName: "RepeatEntity", in: self.context) else {
//                                fatalError("Entity description not found!")
//                            }
//                            let newTask = RepeatEntity(entity: entityDescription, insertInto: self.context)
//                            newTask.repeat_1 = latestMissionValue
//                            self.saveData()
//                        }
//                    }
//                    Thread.sleep(forTimeInterval: 5)
//                } catch {
//                    print("Failed to fetch RepeatEntity objects: \(error)")
//                }
//            
//        }
//    }
    ...
    // Make sure to invalidate the timer when appropriate, e.g., in `deinit` or when the view controller is dismissed
    ...
    override func viewDidLoad() {
        ...
        timer = Timer.scheduledTimer(timeInterval: 2.5, target: self, selector: #selector(updateCellCornerRadius), userInfo: nil, repeats: true)

        CurrentSong.adjustsFontSizeToFitWidth = true
        Current_AI_Presented()
        shuffledSongNames = songNames.shuffled()
        tableView.register(Any_List.self, forCellReuseIdentifier: "MissionCell")
        Song_Voicebox?.delegate = self
        playNextSong()
            ...
            set {
                ...
                if let newYearDate = calendar.date(from: calendar.dateComponents([.year], from: Date())) {
                    if Date() < newYearDate {
                        ...
                        let percentage = Double(newActions) / Double(years_actions_max) * 100.0
                        New_Years_Label.text = "\(newActions) / \(years_actions_max) actions this year \(String(format: "%.2f", percentage))% Finished"
                    }
                }
    ...
    @objc func updateCellCornerRadius() {
        ...
        let totalStages = 4
        let stepSize = (upperBound - lowerBound) / CGFloat(totalStages)
        let elapsedTime = Date().timeIntervalSinceReferenceDate
        let currentStage = Int((elapsedTime / animationDuration).truncatingRemainder(dividingBy: 1.0) * Double(totalStages))
        let fluctuatingRadius = lowerBound + CGFloat(currentStage) * stepSize
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let Gathered_missions = task as? MissionEntity {
            if indexPath.row < Combined_missions.count {
                let task = Combined_missions[indexPath.row]
                if let total_missions = task as? MissionEntity {
                    cell = tableView.dequeueReusableCell(withIdentifier: "MissionCell", for: indexPath) as? MissionCell ?? UITableViewCell()
                } else {
                    cell = tableView.dequeueReusableCell(withIdentifier: "MissionCell", for: indexPath) as? MissionCell ?? UITableViewCell()
                }
            } else {
            ...
            cell.textLabel?.text = "\(Gathered_missions.mission_1 ?? "") \(Gathered_missions.mission_2 ?? "") \(Gathered_missions.mission_3 ?? "") \(Gathered_missions.mission_4 ?? "") \(Gathered_missions.mission_5 ?? "")"
    ...
    @IBAction func addTaskButtonTapped(_ sender: Any) {
        ...
        alertController.addAction(UIAlertAction(title: "Add", style: .default, handler: { [weak self] (action) in
            ...
            for mission in missions {
                self.addTaskOperation(mission1: mission, context: context)
            }
            self.saveData()
            self.fetchData()
        }))
        ...
        guard let soundURL = Bundle.main.url(forResource: "ComputerAlert", withExtension: "wav") else { return }
        do {
            General_Voicebox = try AVAudioPlayer(contentsOf: soundURL)
            General_Voicebox?.prepareToPlay()
            General_Voicebox?.volume = 0.7
            General_Voicebox?.play()
        } catch {
        }
    }
