...
var deleteSoundOptions = ["@*!+-R", "Aflame Procrasination", "Assimilation", "Big Purr", "Blasted", "Buzzed Up", "Chalkboard", "Coin Type", "Coming Near You", "Cybernetic Models", "Dealt With", "Deep Space", "Dented Metal", "Fart", "Glass Shatter", "GoReady", "Gyat Chonki-Lonki", "Laser", "Loading Productive Ammo", "Lost Signal", "Mantid\'s Observation", "Miss Computer Email", "Missile Hit", "Nuclear Ending", "PB & J", "Pleased Watching Mantid", "Productive Prep", "Ready for Takeoff",  "Sea Mine", "Signal", "Static", "Submerge", "Type Ship", "Vegan Muncher", "Water Lurk", "Far Outta Here"]
...
public class SHOP: UIViewController, MFMailComposeViewControllerDelegate, UIPickerViewDataSource, UIPickerViewDelegate, UITableViewDelegate, UITableViewDataSource, SKRequestDelegate {
    ...
    public func requestDidFinish(_ request: SKRequest) {
        if isUserSubscribed() {
            // User is subscribed, perform the desired action
            // For example, navigate to a premium feature
        } else {
            // User is not subscribed, show an alert
            showSubscriptionAlert(message: "For customers only. Subscribe to access colored lists and fonts.")
        }
    }
    ...
    @IBAction func Sketches_Selection(_ sender: UIButton) {
        ...
        if let selectedEntity = UserDefaults.standard.string(forKey: "Selected_AI_Entity") {
            ...
            guard let soundURL = Bundle.main.url(forResource: "Rewiring", withExtension: "wav") else { return }
            do {
                Extra_sounds = try AVAudioPlayer(contentsOf: soundURL)
                Extra_sounds?.prepareToPlay()
                Extra_sounds?.volume = 0.7
