...
var deleteSoundOptions = ["@*!+-R", "Aflame Book Burners", "Assimilation", "Audio Interference", "Beyond the Lies", "Big Purr", "Blasted", "Buzzed Up", "Chalkboard", "Coming Near You", "Cybernetic Models", "Dealt With", "Dented Metal", "Fart", "Glass Shatter", "Gold", "Gyat Chonki-Lonki", "Laser", "Loading Productive Ammo", "Miss Computer Email", "Missile Hit", "Nuclear Ending", "PB & J", "Pleased Observing Mantid", "Productive Prep", "Ready for Takeoff", "Sea Mine", "Signal", "Submerge", "Toxic Combo Book burning discrediting lying gaslighters", "Type Ship", "Vegan Muncher", "W.A.S.P", "Water Lurk", "Within the void"]
...
public class SHOP: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate, UITableViewDelegate, UITableViewDataSource, SKRequestDelegate, SKPaymentTransactionObserver, SKProductsRequestDelegate {
    ...
    public func paymentQueue(_ queue: SKPaymentQueue, updatedTransactions transactions: [SKPaymentTransaction]) {
        for transaction in transactions {
            ...
            case .purchased, .restored:
                // Receipt validation for purchased or restored transactions
                validateReceipt(for: transaction)
                SKPaymentQueue.default().finishTransaction(transaction)
            case .failed:
                SKPaymentQueue.default().finishTransaction(transaction)
            case .deferred, .purchasing:
                // Handle deferred or purchasing state if needed
                break
            @unknown default:
    ...
    private func validateReceipt(for transaction: SKPaymentTransaction) {
        guard let appStoreReceiptURL = Bundle.main.appStoreReceiptURL,
            ...
            // No receipt found, handle the situation appropriately
            return
        }
        
        // Load the receipt data
        guard let receiptData = try? Data(contentsOf: appStoreReceiptURL) else {
            // Error loading receipt data, handle the situation appropriately
            return
        }
        
        // Perform receipt validation using receiptData and your app-specific shared secret
        let appSpecificSharedSecret = "7208390e598445d4aee9637d4e6d008b" // Your actual app-specific shared secret
        
        // Example validation using Apple's verifyReceipt endpoint
        let receiptValidationURLString = "https://sandbox.itunes.apple.com/verifyReceipt" // Use "https://buy.itunes.apple.com/verifyReceipt" for production
        guard let receiptValidationURL = URL(string: receiptValidationURLString) else {
            // Invalid receipt validation URL, handle the situation appropriately
            return
        }
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            guard let data = data, error == nil else {
                // Error during receipt validation request, handle the situation appropriately
                return
            }
            
            // Parse the receipt validation response and handle it accordingly
            do {
                let jsonResponse = try JSONSerialization.jsonObject(with: data, options: []) as? [String: Any]
                // Process the receipt validation response
                // For example, check if the subscription is valid and update the user's subscription status accordingly
            } catch {
                // Error parsing receipt validation response, handle the situation appropriately
            }
        }.resume()
    }
...
public override func viewDidAppear(_ animated: Bool) {
    ...
    public override func viewDidLoad() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
            self.AutoPaymentSuggest()
        }

        SKPaymentQueue.default().add(self)
        
        // Fetch subscription product information
